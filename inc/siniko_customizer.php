...
class Siniko_Customizer {
	...
	public function __construct($data) {
		$this->data = $data;
		add_action('customize_register',array($this,'siniko_customize_register'));
	...
	function custom_admin_style($hook) {
		if ( 'customize.php' == $hook || 'widgets.php' == $hook ) {
			wp_enqueue_style( 'siniko-customizer-css', get_template_directory_uri() . '/inc/css/siniko-customizer.css',array());
			wp_enqueue_script( 'siniko-customizer-js', get_template_directory_uri() . '/inc/js/siniko-customizer.js', array( 'jquery' ), '20150611', true );
		}
	}	
	public function siniko_customize_register($customizeObj) {
	...
	public function create_control($value,$key) {
		...
		if ( isset($value['type']) ) {
			...
			switch($value['type']){
				...
				case "switch":
					$args['type'] = $value['type'];
					$inst = new siniko_Customize_Switch_Control($this->customizeObj,$id,$args);
					break;
				case "image":
	...
	public static function siniko_data_load() {

		$data = apply_filters( 'siniko_customizer_data', array());
...
add_action('init','siniko_customizer::siniko_data_load');
...
if(class_exists( 'WP_Customize_control')) {
	...
	class siniko_Customize_Switch_Control extends WP_Customize_Control {
		public $type = 'switch';
		public function render_content() {
			...
			<label>
				...
				<div class="switch_options">
					<span class="switch_enable"><?php esc_html_e('Yes','siniko'); ?></span>
					<span class="switch_disable"><?php esc_html_e('No','siniko'); ?></span>  
